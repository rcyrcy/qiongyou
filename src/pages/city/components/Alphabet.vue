...
export default {
    ...
    computed:{  // 通过计算属性定义数组类型数据  (因为需要数组存储列表好实现字母列表跟城市列表同步)
        letters(){  
            const letters = []
    ...
    methods:{
        ...
        handleTouchStart(){  //当手指触摸时
            this.touchStarts = true
        },
        handleTouchMove(e){  //手指滑动时
            if(this.touchStarts){
                if(this.timer){ //如果已经存在
                    clearTimeout(this.timer) //去除
                }
                this.timer = setTimeout(()=>{ // 否则创造
